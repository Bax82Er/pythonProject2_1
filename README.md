...
Проект поддерживает следующие форматы данных:
- JSON
- CSV
- XLSX

Для каждого формата реализованы отдельные функции для считывания данных.

Новая функциональность
Фильтрация операций по описанию
Добавлена возможность фильтровать банковские операции по описанию с помощью регулярных выражений. Это позволяет пользователям находить нужные операции, даже если формулировка описания варьируется. Пример использования:


filtered_transactions = filter_transactions_by_description(transactions, "перевод")
Эта функция возвращает список операций, содержащих указанное слово в описании.

Подсчет операций по категориям
Также добавлен функционал для подсчета количества операций по различным категориям. Пользователи могут передать список категорий, и функция вернет словарь, где ключи — это названия категорий, а значения — количество соответствующих операций. Пример:


categories = ["перевод", "открытие"]
result = count_operations_by_category(transactions, categories)
print(result)  # {'перевод': 10, 'открытие': 5}
Улучшенный пользовательский интерфейс
Добавлены улучшения в пользовательском интерфейсе, включая возможность выбора формата файла (JSON, CSV, XLSX), фильтрации операций по статусу, сортировку по дате и валюте, а также уточненный вывод результатов.

Эти изменения помогут пользователям легче ориентироваться в большом количестве банковских операций и быстро находить нужную информацию.